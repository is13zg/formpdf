...
def get_files_with_full_paths(directory):
    try:
        ...
        file_paths = [os.path.join(directory, entry) for entry in entries if os.path.isfile(os.path.join(directory, entry))]
        return file_paths
    except OSError as e:
...
def draw_grid(c, a4_width, a4_height, width_pts, height_pts, separation_pts, margin_pts):
    if separation_pts > 0:
        # Вертикальные линии
...
def create_pdf(directory, width, height, output_path='.', file_name='create_pdf', separation=1, margin=5):
    ...
    for image_file in image_files:
        ...
        if x + width_pts > a4_width - margin_pts:
            ...
            if y < margin_pts:
                draw_grid(c, a4_width, a4_height, width_pts, height_pts, separation_pts, margin_pts)
                c.showPage()
